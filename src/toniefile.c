...
static void toniefile_comment_add(uint8_t *buffer, size_t *length, const char *str)
{
    ...
    memcpy(&buffer[*length], &value, sizeof(uint32_t));
    *length += sizeof(uint32_t);
    strcpy((char *)&buffer[*length], str);
    *length += strlen(str);
}
...
static size_t toniefile_header(uint8_t *buffer, size_t length, TonieboxAudioFileHeader *tafHeader)
{
    ...
    tafHeader->_fill.data = malloc(tafHeader->_fill.len);
    memset(tafHeader->_fill.data, 0x00, tafHeader->_fill.len);
    ...
    {
        printf("TAF header size %lu not equal to frame size %u \r\n", dataLength, proto_frame_size);
    }
    else if (dataLength > length)
    {
        printf("TAF header size %lu bigger than buffer %u \r\n", dataLength, proto_frame_size);
    }
    else
    {
...
toniefile_t *toniefile_create(const char *fullPath, uint32_t audio_id)
{
    ...
    toniefile_t *ctx = malloc(sizeof(toniefile_t));
    memset(ctx, 0x00, sizeof(toniefile_t));
    ...
    strcpy((char *)&comment_data[comment_data_pos], "OpusTags");
    comment_data_pos += 8;
    ...
    if (customTags)
    {
        ...
        int comments = 2;
        memcpy(&comment_data[comment_data_pos], &comments, sizeof(uint32_t));
        comment_data_pos += sizeof(uint32_t);

        char *version_str = "version=0.1"; // TODO
    ...
    else
    {
        ...
        memcpy(&comment_data[comment_data_pos], &comments, sizeof(uint32_t));
        comment_data_pos += sizeof(uint32_t);
        toniefile_comment_add(comment_data, &comment_data_pos, "encoder=opusenc from opus-tools 0.1.10"); // TODO
    ...
    memcpy(&comment_data[comment_data_pos], &remain, sizeof(uint32_t));
    comment_data_pos += sizeof(uint32_t);
    memcpy(&comment_data[comment_data_pos], "pad=", 4);
    ...
    while (ogg_stream_flush(&ctx->os, &og))
    {
        ...
        if (fwrite(og.header, sizeof(uint8_t), og.header_len, ctx->file) != og.header_len)
        {
            return NULL;
        }
        /* write the freshly padded block of frames*/
        if (fwrite(og.body, sizeof(uint8_t), og.body_len, ctx->file) != og.body_len)
        {
            return NULL;
        }
...
error_t toniefile_write_header(toniefile_t *ctx)
{
    ...
    if (fwrite(proto_be, sizeof(uint8_t), sizeof(proto_be), ctx->file) != sizeof(proto_be))
    {
...
{
    memcpy(&dst[*dst_used * OPUS_CHANNELS], &src[*src_used * OPUS_CHANNELS], samples * sizeof(uint16_t) * OPUS_CHANNELS);
    *dst_used += samples;
    *src_used += samples;
}
...
error_t toniefile_encode(toniefile_t *ctx, int16_t *sample_buffer, size_t samples_available)
{
    ...
    while (samples_processed < samples_available)
    {
        ...
        if (ctx->audio_frame_used >= OPUS_FRAME_SIZE)
        {
            ...
            {
                eprintf("Not enough space in this block\r\n");
                return ERROR_FAILURE;
            }
            ...
            {
                eprintf("frame count unexpected: %d instead of %d\r\n", frames, OPUS_FRAME_SIZE);
            }
            ...
            if (page_remain < TONIEFILE_PAD_END)
            {
                ...
                while (ogg_stream_flush(&ctx->os, &og))
                {
                    ...
                    if ((prev / TONIEFILE_FRAME_SIZE) != (ctx->file_pos / TONIEFILE_FRAME_SIZE))
                    {
                        ...
                        {
                            eprintf("Block alignment mismatch 0x%08lx \r\n", ctx->file_pos);
                            return ERROR_FAILURE;
                        }
                    }
                }
...
{
    printf("Start ffmpeg for decoding...\r\n");

    // Construct the FFmpeg command based on the input source
    char ffmpeg_command[1024]; // Adjust the buffer size as needed
    snprintf(ffmpeg_command, sizeof(ffmpeg_command), "ffmpeg -i \"%s\" -f s16le -acodec pcm_s16le -ar 48000 -ac 2 -ss %lu -", input_source, skip_seconds);
    ...
    if (ffmpeg_pipe == NULL)
    {
        eprintf("Could not open FFmpeg pipe\n");
        return ffmpeg_pipe;
    }
    return ffmpeg_pipe;
}
...
error_t ffmpeg_decode_audio_end(FILE *ffmpeg_pipe, error_t error)
{
    ...
    int error_code = pclose(ffmpeg_pipe);

    printf("Stopped ffmpeg with error code=%i...\r\n", error_code);
    return NO_ERROR;
}
...
error_t ffmpeg_stream(char *source, char *target_taf, size_t skip_seconds, bool *active)
{
    printf("Encode source %s as TAF to %s and skip %lu seconds\r\n", source, target_taf, skip_seconds);
    ...
    if (!taf)
    {
        printf("toniefile_create() failed, aborting\r\n");
    ...
    *active = true;
    while (*active)
    {
        ...
        {
            printf("Could not decode sample error=%u read=%lu\r\n", error, blocks_read);
            break;
        }
        ...
        if (error != NO_ERROR && error != ERROR_END_OF_STREAM)
        {
            printf("Could not encode toniesample error=%" PRIu16 "\r\n", error);
            break;
        }
    ...
    printf("TAF encoding successful\r\n");
